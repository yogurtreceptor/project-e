...
Phase 2's original foundation is complete and its expanded workspace remains active. Calendar/Event, Task, reminder, Inbox, local scheduler and deterministic automation capabilities use their own identities while sharing normal validation, audit and provenance. An Event represents something that occurs, occurred or is expected to occur at an interval; the initial timed model requires bounded start and end instants. A Task represents work to be performed. Neither requires a Project, and neither is a reminder. Both use standard relationships to connect to any appropriate entity type, including each other. The [Phase 2 workspace](phase_2_workspace.md) owns detailed milestone status and delivery sequence.
...
A Calendar is a first-class local Event grouping and configuration record; it supplies name, colour, an IANA timezone, default duration, ordering, archive state and reminder policy without becoming a second Event store. The protected Birthdays Calendar is a second built-in Calendar kind: each Person birthday synchronises to one linked canonical, yearly recurring all-day Event in it. That Event remains an Event rather than a derived event type, and its lifecycle follows the Person source. Calendar, Event and timed Task forms select the IANA identifier through a local searchable picker; the country and current offset are selection aids and are not canonical values. Archiving a Calendar retains its Event assignments and blocks new selection; it does not move or archive Events. A Calendar with assigned active or recycled Events is not deletable. A calendar view is a derived projection over canonical records and traceable derived occurrences. See [the Phase 2 workspace](phase_2_workspace.md) for the detailed temporal, reminder, scheduler and automation semantics.

An automation rule is an operational configuration record, not a canonical entity, Event, Task or Scheduled Job. It deterministically maps a registered trigger and optional conditions to a registered action. An automation run is one historical execution attempt. A review proposal is a separate operational record that describes a consequential canonical mutation; it is not the canonical mutation and only explicit approval applies its proposed Task through the normal service boundary.
