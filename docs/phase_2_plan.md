...
The initial single-user platform timezone is the IANA zone `Australia/Brisbane` (UTC+10 without daylight saving). Timed records are interpreted and displayed in that zone. Where a precise instant is persisted, storage uses a backend-safe UTC representation together with the platform temporal semantics; Phase 2 does not offer a per-record timezone selector. This preserves a clean boundary for a later multi-zone model without changing the meaning of current records.
