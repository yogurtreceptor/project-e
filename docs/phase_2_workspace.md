...
20. **Complete (2026-07-30):** move Calendar navigation into the Project E header on Calendar pages only. The header contains a compact **Today** control, sleek previous/next chevrons, the current anchor date without its weekday (for example, **30 July 2026**), the existing Calendar-view selector, and a cog affordance for future Calendar settings. The cog looks interactive and exposes **Coming soon!** on hover, but clicking it has no effect until a separately authorised settings workflow is implemented.
