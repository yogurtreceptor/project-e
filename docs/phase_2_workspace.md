...
**Phase 2 — Complete.** The authorised Calendar and Event work, reminder delivery and recovery, scheduled Job Runs, registered deterministic automation, portability and data-quality coverage are delivered. The experimental Task implementation was retired on 2026-08-01 after the local database was verified to contain no Task records. The owner accepted the delivered experience and authorised final milestone closure on 2026-08-03 after reviewing the candidate evidence. Persistent System Health remains deferred; Phase 2 closure does not authorise it.
...
The phase remains human-first, database-first, local-first and AI-independent. This document is the canonical completed Phase 2 scope, architectural direction, delivery record, completion standard and exclusion list. The expansion section records delivered refinement work; historical plans and roadmap entries do not authorise further implementation.
...
66. **Complete (2026-08-01) — direct Other-calendar editing:** give every enabled URL Calendar under **Other calendars** the same accessible vertical-ellipsis edit shortcut already shown for General, Birthdays and other local Calendars. The shortcut opens that source's existing applicable Calendar Settings form directly and preserves the originating Month, Week or Day context and visible-Calendar selection for return; it introduces no duplicate form, persistence path or external-item editing.
...
Journey planning belongs to the [Phase 3 Spatial Intelligence planning workspace](phase_3_spatial_intelligence_planning.md). It was not a Phase 2 completion requirement, and its planning direction does not authorise implementation.

## Phase 2 closeout process

The following process governed the Phase 2 closeout work prepared in PR #6. Steps 1–6 produced the reviewed evidence and completed milestone status; the repository publication and merge operation in step 7 remains a separate action and was not performed by the 2026-08-03 documentation closure. The process did not authorise another Phase 2 feature or implementation of accepted technical debt. All walkthrough and visual-review data remained fictional and confined to isolated, disposable runtime paths.

1. **Freeze the closeout baseline and trace every gate.** Confirm that PR #6 still compares the complete `dev` history with `main`, remains draft and mergeable, and records the same completed evidence and unchecked gates as this workspace. Capture its head commit and the current tracked-tree state so later evidence names the reviewed revision. Treat deferred features and live debt as exclusions or owner decisions, not implementation work.
2. **Reconcile the current contract without erasing history.** Audit current-facing Phase 2, architecture, design, glossary, UI-copy and test claims for Task, System Health, consequential proposals and approval. Update the authoritative current-behaviour and completion language to describe the Task-free product while retaining dated delivery entries, append-only migrations, superseded ADR material and the build log as historical evidence. Amend ADR-017 so closure covers the delivered Event, Calendar, reminder, Inbox, scheduler, registered-automation, audit, provenance and portability system with Persistent System Health deferred. Amend ADR-020 so the only current registered automation is non-consequential, registry-bound, contains no database- or user-authored executable code and cannot mutate a canonical Event; any future consequential action requires separately authorised design and review infrastructure. Correct the System Tools description and every other present-tense claim that users can approve automation proposals, then run the focused documentation/rendering, automation and migration tests affected by those edits.
3. **Create repeatable integrated closeout evidence on an isolated database.** Prefer one dedicated deterministic closeout test or fixture-driven walkthrough so the evidence can be rerun. Create fictional Project, People and Location records; create and relate an Event; add recurrence and one occurrence exception; exercise either the Birthday Event or Document-expiry occurrence route and cite existing focused coverage for the other; combine Calendar-default and Event-level reminder policy; deliver one local Inbox item; simulate startup recovery twice and prove no duplicate delivery; execute the scheduled reminder job and its registered automation; and demonstrate that Job Run, Automation Run, Inbox delivery/action and audit records retain separate identities. Exercise Event cancellation, archival and Recycle Bin/permanent-deletion semantics as distinct states without conflating canonical records, derived occurrences or projections. Export the resulting platform bundle, validate it, import it into another empty isolated database and verify representative identities, relationships and operational history. Record the reviewed revision, setup, expected result, actual result and stable counts or identifiers in the closure review without retaining the databases or export bundle.
4. **Perform representative human visual and keyboard review.** Run the application against the fictional isolated database and inspect Calendar Month, Week and Day, Calendar Settings, Inbox, Scheduled Jobs and Deterministic Automation. Cover the Calendar's constrained 800 × 600 baseline and the ordinary 1440 × 900 and 1920 × 1080 desktop baselines, including both supported colour-scheme states where the design contract requires them. Check keyboard-only reachability, logical tab order, visible focus, accessible control names, activation and return context, independent Calendar grid scrolling, content clipping/overlap and the legibility of status, history and empty states. Record pass/fail evidence and accepted limitations; route residual non-blocking defects to the technical-debt register instead of broadening closeout into a redesign.
5. **Prepare the final closure review and owner decision pack.** Replace `docs/reviews/phase_2_exit_review.md` with a dated current-state review that identifies the exact revision, the Task-free delivered boundary, the integrated walkthrough, visual/keyboard results, automation safety boundary, lifecycle and identity distinctions, portability result, test evidence, accepted debt, explicit exclusions and any residual limitations. Keep the closure decision pending until the user has reviewed that evidence. Present the five owner gates below as explicit decisions: milestone meaning, delivered experience, accepted debt, deferred boundary and final authority to mark Phase 2 complete and merge PR #6. Record only approvals the user actually gives.
6. **Apply the approved completion state and perform final verification.** After the owner authorises closure, change Phase 2 from active/closeout wording to complete in `AGENTS.md`, `PROJECT_GOAL.md`, `README.md`, `ROADMAP.md`, current architecture documentation and this workspace, while preserving historical Phase 2 records and keeping later planning distinct from implementation authority. Run `python3 -m unittest discover -s tests`, `python3 -m compileall app run.py tests` and `git diff --check`; inspect the tracked tree and pending diff for databases, exports, logs, caches, credentials, personal data and other runtime artifacts. Record the final revision, test count, compile result, artifact check and approved closure decision in the final review.
7. **Synchronise and merge through the gate.** Update the workspace checklist and PR #6 body together as each gate is satisfied, keep the PR draft and unmerged while any repository item or owner approval is outstanding, and recheck the final head against the reviewed evidence. Once every checkbox is complete, publish the final closeout commits, make the PR ready for review, confirm it remains mergeable and that its checks pass, then merge the complete `dev` history into `main` using the repository's history-preserving merge route. Verify `main` contains the reviewed head and leave ordinary residual defects in normal maintenance rather than reopening Phase 2.

## Phase 2 closure record

This checklist records closure of Phase 2 as a **development milestone**, using the same representative-evidence standard as Phase 1. Closure does not assert exhaustive testing, a stable public release or the absence of residual defects. Later defects are handled through ordinary maintenance and do not reopen Phase 2 as a whole unless the product boundary itself is deliberately reconsidered. No item below authorises another feature merely to make the phase appear larger.

### Completed repository evidence

- [x] Every authorised Phase 2A–2F implementation-sequence item is recorded complete, including the original integrated stabilisation review.
- [x] Every authorised Event-focused expansion through entry 66 is recorded complete, including Calendar refinement, recurrence, iCalendar interchange, read-only URL Calendars, Inbox refinement and direct Other-calendar editing.
...
- [x] After the approved completion-status and Phase 3 planning edits, the full **287-test** suite, compile check, diff check and artifact inspection passed again on 2026-08-03.
...
- [x] Replace the earlier completion review with a dated final closure review describing the current Task-free product, final evidence, accepted debt, exclusions and closure decision.
- [x] Change Phase 2 from active/closeout wording to complete in `AGENTS.md`, `PROJECT_GOAL.md`, `README.md`, `ROADMAP.md`, architecture documentation and this workspace; define Phase 3 planning separately without treating it as implementation authority.
...
- [ ] Publish and merge the complete `dev` history through the Phase 2 pull request when separately requested. Milestone completion does not imply that this repository operation has already occurred.
...
- [x] **Delivered experience:** on 2026-08-03 the user confirmed that the current Calendar/Event, reminder/Inbox and local operational-runtime experience may be left in place as the completed Phase 2 foundation.
...
- [x] **Final closure:** on 2026-08-03 the user explicitly authorised marking Phase 2 complete and aligning the repository documentation. No branch merge was requested or performed by this closure pass.

## Completion criteria

Phase 2 completed after the end-to-end review demonstrated:
