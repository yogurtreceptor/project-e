...
### Planned Inbox reminder refinement — awaiting implementation approval

This brief records the agreed direction and a reviewable implementation sequence. It is planning context, not implementation authority, and no item below is **Complete** until the user approves the work and the delivered behaviour is verified.
...
1. If an earlier reminder for one occurrence has been opened or dismissed, should a separately configured later timing still appear when it becomes due? The proposed default is **yes**: only one reminder is visible at a time, but distinct configured timings remain meaningful.
2. Should **Open event** land on that occurrence in Calendar context, or on the Event's read-only record page? Calendar occurrence context is the proposed default, particularly for recurring and read-only external occurrences.
3. Should reducing routine no-op Job Run, automation-run and global-audit noise be included with this work, or assessed as a separate runtime-history refinement? It does not affect Inbox correctness and may change the deliberately durable execution-history contract, so the proposed default is a separate follow-up.
