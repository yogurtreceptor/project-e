...
42. **Planned — selectable multi-Calendar ZIP export:** replace the Export placeholder with a colour-and-name checklist of local Calendars. Every active Calendar is selected by default, with **Select all** and **Clear all** controls; archived Calendars appear in a separate unchecked group but remain explicitly selectable. The selection is independent of Calendar-view visibility and must contain at least one valid Calendar. A single read-only export returns `application/zip` with one independently valid, safely and uniquely named `.ics` member per selected Calendar, including an empty `VCALENDAR` for a selected Calendar with no exportable Events. Each member carries its own `VCALENDAR` metadata, `X-WR-CALNAME`, `X-WR-TIMEZONE`, active non-recycled and non-archived assigned Events, whole-Event cancellation status and recurrence definitions that the first supported subset can express exactly. Preserve imported UIDs; generate deterministic Project E UIDs for native Events so repeated exports retain identity. Escape text, use CRLF content lines, fold at the iCalendar byte limit, emit all-day exclusive `DTEND`, and serialize recurrence without materialising occurrences. The ZIP is an interoperability convenience containing ordinary iCalendar files, not a Project E backup and not a replacement for whole-platform portability. Validate every selected Calendar before starting the download; if any selected Calendar contains timed Events, occurrence exceptions or another unsupported semantic, show a precise per-Calendar compatibility report and produce no partial or lossy ZIP.
