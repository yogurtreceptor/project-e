...
37. **Planned — preview-first Calendar creation:** replace the Import placeholder with an upload form and a validation preview. The preview derives the proposed Calendar name from `X-WR-CALNAME`, falling back to the safe filename stem, and derives its IANA timezone from `X-WR-TIMEZONE` when present. It shows the proposed name, colour, timezone, Event count, recurring count, date span, per-Event mapping and every warning or unsupported feature. Name, colour and timezone remain editable before confirmation, and a conflicting Calendar name requires an explicit unique replacement. The uploaded bytes are staged only under Git-ignored runtime storage behind a random, single-use, expiring token. Preview and cancellation perform no canonical writes.

38. **Planned — first supported Event mapping:** on confirmed apply, create a normal local Calendar and map `SUMMARY` to Event title, `DESCRIPTION` to notes, date-only `DTSTART` plus exclusive `DTEND` to Project E's existing all-day interval, `STATUS:CONFIRMED` to planned and `STATUS:CANCELLED` to cancelled. A missing all-day `DTEND` means a one-day Event. Map only recurrence rules that the canonical recurrence model represents without loss: `FREQ` daily, weekly, monthly or yearly; positive `INTERVAL`; supported weekly/ordinal `BYDAY`; and a supported `UNTIL` or `COUNT` boundary. The sample's `RRULE:FREQ=YEARLY` therefore becomes one canonical Event series and derived yearly occurrences, not one materialised Event per year.
...
41. **Planned — atomic and authorised apply:** confirmation is the explicit approval to create the proposed canonical Calendar and its listed canonical Events. Reparse and revalidate the staged bytes and editable Calendar settings at apply time; do not trust hidden preview counts. Extend the existing Calendar/Event/recurrence service boundaries with transaction ownership and `imported` provenance rather than duplicating SQL. Calendar creation, Events, recurrence definitions, interchange identities, field provenance, per-record history/audit and one summary import audit record commit in a single transaction; any failure rolls back the whole import. Applying an iCalendar file does not create a subscription, retain a WAN URL, schedule refreshes or later overwrite user edits.

42. **Planned — selected-Calendar iCalendar export:** replace the Export placeholder with a selector for one local Calendar and return a read-only `text/calendar; charset=utf-8` download with a safe `.ics` filename. Export `VCALENDAR` metadata, `X-WR-CALNAME`, `X-WR-TIMEZONE`, active non-recycled and non-archived Events assigned to that Calendar, whole-Event cancellation status and recurrence definitions that the first supported subset can express exactly. Preserve imported UIDs; generate deterministic Project E UIDs for native Events so repeated exports retain identity. Escape text, use CRLF content lines, fold at the iCalendar byte limit, emit all-day exclusive `DTEND`, and serialize recurrence without materialising occurrences. Archived/recycled Events remain whole-platform portability data rather than Calendar-interchange output. If the selected Calendar contains timed Events, occurrence exceptions or another unsupported semantic, stop with a precise compatibility report instead of producing a lossy file.

43. **Planned — testing and delivery evidence:** cover parser unfolding/escaping, malformed input, limits, date boundaries, multi-day spans, yearly recurrence, unsupported-property blocking, preview-without-writes, single-use staging, explicit confirmation, atomic rollback, imported provenance/audit, fresh and upgraded schemas, UID repeat/conflict behaviour, safe download headers/names and supported import→export→parse equivalence. Add focused route/rendering tests and a live upload/preview/confirm/download smoke test using a temporary database. The local Neo-Pagan sample may be used manually as acceptance evidence but must never be copied into tracked fixtures, runtime examples or commits. Before timed Event interchange is claimed, inspect at least one Google export containing UTC, `TZID` and preferably daylight-saving behaviour; before exception support is claimed, inspect one recurring series with a deleted occurrence and one rescheduled occurrence.
