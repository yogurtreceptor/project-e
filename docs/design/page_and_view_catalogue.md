...
| `/calendar/settings/from-url` and `/calendar/settings/other-calendars/{id}` | Preview and confirm a public-HTTPS URL Calendar; inspect, refresh, enable, disable or remove each existing source from its individual settings page | `calendar_settings_from_url_page()`, `calendar_settings_subscription_page()` | Existing sources live under **Settings for other calendars**, not a generic Subscriptions collection. Their items remain read-only and non-canonical; projection and export consume a last-known-good cache rather than fetching during Calendar renders. |
