...
| `/inbox` and `/inbox?archived=1` | Chronological reminder attention; open exact Event occurrence or Document; dismiss; snooze ten minutes; browse retained delivery/action history | `inbox_page()` | Current active scope is reminders only. One occurrence/reason is visible once per configured timing and at most one timing is active; Event attention clears on open/end while Document-expiry attention persists. No Upcoming preview, manual evaluation or acknowledgement action is exposed. |
| `/inbox/count` | Read-only JSON count of currently visible reminder attention | `inbox_count()` | Supports the semantic Browse-navigation badge through visibility-aware local polling; it creates no delivery and is not a notification channel. |
...
| `/calendar/settings/from-url` and `/calendar/settings/other-calendars/{id}` | Preview and confirm a public-HTTPS URL Calendar; edit its name, colour, timezone and Calendar-level notifications; inspect, refresh, enable, disable or remove it | `calendar_settings_from_url_page()`, `calendar_settings_subscription_page()` | Existing sources live under **Settings for other calendars**, not a generic Subscriptions collection. Default Event duration and item-level controls are absent because their items remain externally owned, read-only and non-canonical. Projection, reminder evaluation and export consume the last-known-good cache rather than fetching during Calendar renders. |
