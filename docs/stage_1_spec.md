...
- Entity create and edit forms provide **Add details** for definition-driven optional data. Added controls appear inline in canonical field order rather than in a separate section; populated details are visible by default, and hiding a detail does not clear its value. Coordinates are added as a compound latitude/longitude detail and require both values. People expose Alias, Nickname, Height, Weight, Languages, Nationalities and Ethnicities this way. Organisations progressively disclose contact fields; Locations disclose detailed address, coordinates and source fields; Projects disclose dates; Documents disclose dates, identifier and legacy issuer text; Assets disclose manufacturer, model, serial, acquisition, value and coordinates. Populated details display and participate as normal entity data.
...
- Documents may store an optional identifier/reference number and expiry date. Projects may store an optional ended/completed date. Document expiry and Project end dates feed the Universal Timeline.
