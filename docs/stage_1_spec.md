...
- System Tools can export a versioned, checksummed Project E Bundle containing a PostgreSQL-native snapshot and referenced uploaded files. Import validates an isolated restore, schema, relationships, checksums and document membership, previews counts, requires an empty target and explicit confirmation, creates a recovery backup, and records the import in audit history.
