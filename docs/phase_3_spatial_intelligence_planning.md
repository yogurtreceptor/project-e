...
**Phase 3 is in progress, with implementation complete through N3 and the X1 prerequisite evidence complete.** N1, N2 and N3 were delivered and verified on 2026-08-05; X1 was completed on 2026-08-07 without installing a production provider or pack. N4–N9 and L1–L3 remain planned, not delivered. Dated **Complete:** entries are delivered contracts or evidence; all other outcomes, ordering and candidates remain planning that does not authorise implementation or claim those capabilities exist.
...
| N4–N9 and L1–L3 | **Planned · not implemented** | Installed-region maps, production journeys, Calendar materialisation and later spatial capabilities. |
...
The current platform has canonical Locations with address and role-based geometry assertion history, explicit containment Relationships, the Map 2.0A canonical workspace with local coordinate rendering/search/state and explicitly requested Nominatim search, the provider-independent journey request/result/failure seam with durable mobility-profile/routing-policy identity and a disposable cache boundary, and the Phase 2 Event/Calendar/reminder/Inbox/scheduler foundation. X1 now supplies reproducible Gold Coast source/build measurements and capability-specific candidates for the next slices. The application still has no installed map/search pack, production routing adapter, calculated production route, reviewed production profile/policy value or journey group.
...
| Offline basemap | tilemaker-derived vector MBTiles plus vendored MapLibre GL JS as the N4 front-runner; PMTiles remains a storage challenger pending lifecycle/serving evidence |
| Local search | MOTIS geocoding as the N4 front-runner; a derived SQLite/FTS index is the capability-separation challenger |
...
The Gold Coast spike created the official 15 km buffered boundary route, verified one portable source manifest, retained SEQ GTFS separately, built fingerprinted tile and graph evidence, and tested internal/boundary routes. It also showed that `complete_ways` extraction and MOTIS import each peak around 3.2 GiB on the measured host, so N4 begins with prebuilt verified derivatives and must measure staged disk duplication before setting budgets. Later Logan/Brisbane packs still use the same source-snapshot contract; compatible source packs rebuild one union graph rather than overlay separately built engine graphs. The full measurement and recovery record is [Phase 3 X1 Gold Coast Spatial Evidence](reviews/phase_3_x1_spatial_evidence.md).
...
The evidence selects capability-specific front-runners rather than one provider: tilemaker/MBTiles plus vendored MapLibre for N4 display, MOTIS geocoding for N4 installed search, Valhalla for N6 street routing and MOTIS for N7 static transit. These remain reversible starting positions, not installed architecture. Missing coastline/style/browser proof, MapLibre's need for N2's DOM/text accessibility layer, MOTIS's coupled import peak/default direct-route bound and absent explicit Wait legs, Valhalla's distant out-of-coverage snapping and missing admin/timezone side data, malformed-update validation and native Valhalla Windows packaging are explicit N4/N6/N7 obligations and reconsideration triggers. No N3 contract change was needed. Licences, attribution, mutable-source retention/reacquisition, last-known-good rollback and exact tool/source/build digests are recorded in [the X1 evidence report](reviews/phase_3_x1_spatial_evidence.md).
...
| --- | --- | --- |
| D01, D06, D16, D18 | **X1 evidence available; N4 lifecycle decision remains.** Gold Coast source/build digests, a 15 km envelope, prebuilt-first direction, licences/reacquisition, reference-completion behaviour and measured build size/time/memory are recorded. N4 must still prove archive/directory shape, coastline completion, raw-input retention, staged disk budget, interrupted/overlap rollback, a bordering-suburb cut and later Logan/Brisbane union before fixing production rules. | Gold Coast first; portable source packs plus disposable derived builds; independent LGA/suburb units; route coordinates survive missing map coverage; contextual sidebar offers nearby suburb, LGA and online capability choices but never auto-installs or silently claims absent routing capability. |
...
| D07, D08, D14 | **N2 foundation resolved; X1 candidates measured.** tilemaker/MBTiles plus vendored MapLibre is the N4 normal-map front-runner and MOTIS geocoding is the installed-search front-runner. Coastline/style/browser/accessibility and activation proof remain N4; satellite/terrain, specialist overlays, current-location implementation/consent and online connection packaging remain later slices. | Build incrementally; search results never rerun on pan while enabled layers load visible features; persistent accessible red selection pin/sidebar; blank clicks do not pin; clickable provider features never mutate; current location is a Phase 3 deliverable with device-first one-shot use and optional disclosed IP fallback; no tracking or hidden query logging. |
| D19 | **N2 Map-state subset resolved.** Recent-search/selection retention, named-list/favourite schema, provider-reference reconciliation and portability remain. They need external bookmark disappearance/refresh, list revisit and clear/export workflows; last viewport/layer state, sidebar navigation and shared-place grouping are delivered. | Normal base, canonical Locations and available general-place/transit features begin visible; other canonical/specialist layers begin off and choices persist locally. Recents are bounded and clearable; external bookmarks need no canonical promotion; user membership is portable while provider facts are reacquired; no reusable saved journey. |
