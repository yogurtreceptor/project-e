...
Migration `20260723_23_reminder_and_inbox_foundation` adds `reminder_policies`, record/occurrence `reminder_overrides`, and durable `inbox_items`. Policy and override timings are stored as validated JSON timing tokens, not user-entered comma-separated text; forms submit each timing as an integer/unit pair before the service produces its canonical token. The service resolves source defaults with local Calendar, URL Calendar or global derived-source policy where present, custom additions and individual suppression. Calendar policies contain at most ten timings, and canonical Event override validation limits the resulting effective set to ten. An absent ordinary Calendar policy inherits the platform Event defaults, while the protected Birthdays Calendar inherits the longer Birthday defaults; a canonical Event with no custom additions or suppressions inherits its linked local Calendar policy, and a cached URL Calendar occurrence has no record-level override and uses its URL Calendar policy. Saving an empty Calendar notification creator restores platform inheritance. Inbox delivery keys are unique hashes of source, logical occurrence, due instant, timing and reason, so repeated scheduled evaluation cannot duplicate a delivery while a changed due instant has a new identity. Inbox state preserves active, snoozed, acknowledged, dismissed and resolved history; the current interaction uses dismiss and fixed ten-minute snooze while historic states remain readable.
...
Migration `20260801_32_consolidate_inbox_attention` adds `attention_expires_at` to `inbox_items`, reconciles pre-existing active/snoozed timing duplicates by retaining the latest due threshold, records each supersession in action history and creates a partial unique index over source kind, source identifier, occurrence key and reason for active/snoozed states. Event-like deliveries store their occurrence end instant and scheduled evaluation resolves them after that boundary; persistent Document-expiry attention stores an empty boundary. Separate timing-specific delivery keys remain historical identities, so a later configured timing may appear once after an earlier timing was opened or dismissed without redelivering either timing.
...
ADR-028 requires record-derived reminder sources to supply shared occurrence identity, source destination, due/end boundaries and source-specific persistence/resolution behaviour through `app/temporal_occurrences.py`. It does not require one universal persistence table: canonical source tables retain their dates, while occurrence adapters and projections remain deterministic unless a separately authorised workflow deliberately materialises a canonical Event. Calendar and reminder services consume the registered providers, and the shared reminder-policy and delivery tables remain the downstream persistence boundary. A future source extends this provider registry rather than adding a domain branch to the evaluator.
