...
Phase 2 is planned and introduces no current schema. Its future persistence design must preserve SQLite as the canonical store and use migration-safe evolution. Events and Tasks will be canonical first-class entity records connected through the existing relationship model. Calendars are first-class local Event grouping/configuration records with name, colour, IANA timezone, default Event duration and reminder preferences; every Event selects one calendar. Calendar views are projections over canonical records or traceable derived occurrences; they are never an independent event source of truth.

The shared temporal model must use compatible semantics and utilities for instants, intervals, all-day values, time zones, deadlines, recurrence, uncertainty, cancellation and rescheduling; it does not require a premature universal base table. Initial Events and Tasks store planned time only; actual start/end tracking is a deferred extension for concrete time-tracking workflows. Timed Events always persist start and end instants; all-day Events persist date boundaries only. Intervals are start-inclusive and end-exclusive. Calendar settings provide overridable default Event duration and reminder preferences. Initial date precision is exact or approximate: an approximate date stores the user's closest known date plus its precision marker, not a date range or partial date. The initial platform timezone is `Australia/Brisbane`; precise persisted instants use backend-safe UTC representation. Calendars supply IANA timezone defaults and individual Events may override that timezone while retaining their precise UTC instant. Recurrence must support calendar-grade interval, weekday, ordinal-weekday and bounded-date-range patterns. Generated occurrences remain traceable rather than duplicate Event rows; occurrence-specific, prospective-series and entire-series changes must be separately traceable. Reminder definitions are attached policies and overrides, while acknowledgement, snooze, delivery failure and delivery history are separate notification-delivery records. Initial delivery is local-inbox-only; an unavailable application's startup recovery creates one deduplicated item for an otherwise missed due notification. Persistent health conditions, inbox notifications, audit events and job-run history are separate durable concepts. Stable identities identify logical occurrences, deliveries, recoveries, issues and escalations; database uniqueness constraints plus atomic claims protect future external side effects from concurrent duplicate execution. A recurrence change creates new future identities, a reminder timing/policy change creates a new pending identity, disabling a reminder suppresses pending delivery, and re-enabling delivers only when due under its current policy. Issue severity escalation, notification re-due state and individual job-run identity follow the documented material-change contract.
