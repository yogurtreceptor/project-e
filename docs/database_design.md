...
Phase 2 is complete and preserves SQLite as the canonical store through migration-safe evolution. The current model uses `calendars` as the sole Event grouping and configuration store. A fresh installation receives one default General Calendar with colour `#2563EB`, timezone `Australia/Brisbane`, a 60-minute default Event duration and ordering value zero. Calendars have stable identifiers, case-insensitive unique names, colour, timezone, default duration, ordering, timestamps, archive state and a database-enforced single default. Event colour derives from the Calendar; there is no Event-specific colour override.
