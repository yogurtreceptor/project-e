...
Phase 2 is in progress and preserves SQLite as the canonical store through migration-safe evolution. Migration `20260719_16_temporal_foundation` creates local `calendars` and `event_categories` reference records. A fresh installation receives one default Calendar (`Australia/Brisbane`, 60-minute default duration) and one default General category. Both stores have stable identifiers, case-insensitive unique names, colours, timestamps and archive state; partial unique indexes enforce at most one designated default. Reminder preference columns are deliberately deferred until the unresolved precedence among Calendar, category, global and record-level policies is approved.

Events and Tasks will be canonical first-class entity records connected through the existing relationship model. Every Event will select one Calendar and one category. Calendar views are projections over canonical records or traceable derived occurrences; they are never an independent event source of truth.
...
Migration `20260719_17_canonical_events` adds `event` to canonical entity identity and creates the `events` typed table. Each Event references one Calendar and one Event category with restricted deletion. Timed rows store a bounded UTC start/end pair plus the originating IANA timezone; all-day rows store date-only start and exclusive-end boundaries. A database constraint makes these temporal modes mutually exclusive. Date precision is `exact` or `approximate`; status is currently `planned` or `cancelled`; `archived_at` remains distinct from both cancellation and the entity Recycle Bin's `deleted_at`.

`app/event_service.py` is the Event write boundary. It applies defaults only at creation, permits an existing Event to retain an archived Calendar or category while preventing new assignment to archived reference data, and writes audit, provenance and entity-edit history through existing platform stores. Event identity is stable through edits and archive/unarchive operations. Event creation is deliberately not exposed through generic entity forms.

The shared temporal model must extend these compatible semantics for deadlines, recurrence, uncertainty, cancellation and rescheduling; it does not require a premature universal base table. Initial Events and Tasks store planned time only; actual start/end tracking is a deferred extension for concrete time-tracking workflows. Calendar settings provide overridable default Event duration and reminder preferences. Initial date precision is exact or approximate: an approximate date stores the user's closest known date plus its precision marker, not a date range or partial date. Recurrence must support calendar-grade interval, weekday, ordinal-weekday and bounded-date-range patterns. Generated occurrences remain traceable rather than duplicate Event rows; occurrence-specific, prospective-series and entire-series changes must be separately traceable. Reminder definitions are attached policies and overrides, while acknowledgement, snooze, delivery failure and delivery history are separate notification-delivery records. Initial delivery is local-inbox-only; an unavailable application's startup recovery creates one deduplicated item for an otherwise missed due notification. Persistent health conditions, inbox notifications, audit events and job-run history are separate durable concepts. Stable identities identify logical occurrences, deliveries, recoveries, issues and escalations; database uniqueness constraints plus atomic claims protect future external side effects from concurrent duplicate execution. A recurrence change creates new future identities, a reminder timing/policy change creates a new pending identity, disabling a reminder suppresses pending delivery, and re-enabling delivers only when due under its current policy. Issue severity escalation, notification re-due state and individual job-run identity follow the documented material-change contract.
