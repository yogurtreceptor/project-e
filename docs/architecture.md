...
Phase 1's information model and main human-facing platform are complete as a development milestone. Phase 2 is in progress and not yet complete. `app/temporal.py` owns IANA-timezone wall-time normalization, precise UTC instants and end-exclusive all-day intervals. Calendars are the sole Event grouping/configuration records and own Event colour, default timezone, default duration, ordering and archive state. `app/event_service.py` owns canonical Event validation, persistence, detail edits, dedicated cancellation/reinstatement/rescheduling, and archive operations over shared `entities` identity and Event-specific storage. Event is registered as a canonical type for schema integrity, portability and future relationships, but remains outside generic browsing and creation because human-created Events must originate from the Calendar workflow. Calendar UI, relationship/search integration, projections and recurrence remain subsequent Phase 2A work.
