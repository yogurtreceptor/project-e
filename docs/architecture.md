...
`app/graph_layout.py` provides a presentation-neutral layered layout. Edges declare a rank difference and connector style, allowing the same layout to support future organisation, project, document or mixed-entity graphs without family-specific rendering rules. Zero-rank-connected nodes are grouped onto the same row and kept adjacent. Endpoints that connect to the same opposite node with the same rank delta are also aligned, so missing nodes leave generational space instead of pulling a shorter branch upward. Positive-rank edges then establish the generational grid. Deterministic neighbour-based ordering sweeps place each node near its actual adjacent-row connections, and differing hierarchy-neighbour sets receive extra horizontal separation to avoid visually implying unstored links. Positive-rank cycles are detected and marked rather than repeatedly traversed, and nodes are keyed by canonical entity ID. Server-rendered SVG uses straight horizontal peer connectors and orthogonal hierarchy connectors, with edge metadata selecting visual weight and dash style. Hierarchy targets are bundled only when their complete incoming-source sets match. Every distinct source combination receives independent source-node ports, routing lanes, trunks and target bars; generation spacing expands with bundle count. Connector geometry therefore never shares a stem or junction across different endpoint sets, and panel-coloured casing makes unavoidable crossings read as overpasses rather than junctions, preventing blended branches from visually claiming extra relationships.

The proof of concept favours deterministic, understandable placement over pedigree-chart optimisation. It does not infer unstored relationships, merge spouses into family units or guarantee ideal ordering in dense/disconnected graphs. Future visualisations should add an extractor (and, where necessary, a different reusable layout strategy) while retaining the same canonical graph contract.
