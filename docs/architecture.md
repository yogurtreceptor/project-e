...
These are current implementation boundaries. Phase 3 Spatial Intelligence is in planning and has not yet changed the architecture described here. AI and agent layers are postponed indefinitely as a primary focus; any small bounded future capability still requires explicit authorisation and must consume shared validation, relationships, provenance, audit history, data quality and safe domain operations. SQLite remains the canonical source of truth.
...
The delivered Phase 1 and Phase 2 milestones did not redesign Locations, migrate Location countries or provenance, integrate G-NAF, expand contact methods, redesign Asset value/currency, add currency conversion, or broaden taxonomies beyond their required relationship work. Phase 3 now supplies a concrete spatial-planning context for reconsidering some Location and map deferrals, but the planning workspace does not itself alter this current architecture. Journal generalisation is separately described above and remains unimplemented.
...
Proposed richer geometry, regional spatial data packs, Map 2.0, routing and journey-planning boundaries belong to the [Phase 3 Spatial Intelligence planning workspace](phase_3_spatial_intelligence_planning.md). Until an implementation is explicitly authorised and delivered, the current point-marker, relationship-derived and optional-network architecture above remains authoritative.
