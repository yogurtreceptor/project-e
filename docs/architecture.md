...
`app/journey_repository.py` owns audited user configuration identity and revisions for mobility profiles and routing policies. N6 keeps Regular walk, Fast walk/jog and Run as stable Walk-profile identities but requires three reviewed user measurements before any preset becomes usable; it stores the median speed and optional contiguous distance/duration limits without silently learning from routes. The only production policy mapping is an explicitly enabled strong soft preference to avoid steps; it is never described as an accessibility guarantee.

`app/routing_resources.py` owns the ignored production-routing activation boundary. It accepts only the exact X1-reviewed Valhalla 3.8.3 executable, loopback configuration and 46,018,560-byte Gold Coast graph digests, the matching `au-qld-gold-coast` Spatial Pack version and reviewed coverage/source/cache declaration. `app/valhalla_adapter.py` rechecks resource bytes before launch, uses only `127.0.0.1`, sends coordinates/measured speed/supported controls rather than canonical labels, independently limits endpoint snapping to 100 metres and validates returned route geometry against the declared extent. Provider time is treated as a static duration estimate; Project E applies Australia/Brisbane depart-at/arrive-by and named-buffer semantics because the graph lacks reviewed time-dependent/admin data.
