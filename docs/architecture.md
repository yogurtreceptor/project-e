...
`app/journey_repository.py` owns audited local configuration identity and revisions for mobility profiles and routing policies. N6 currently initialises one provisional Regular walk profile at 5 km/h on first use, sourced as a generic Google Maps Community Product Expert estimate rather than a personal measurement or an official published Google constant. Fast walk/jog and Run remain visibly unavailable and are rejected by both the form contract and adapter. The only production policy mapping is an explicitly enabled strong soft preference to avoid steps; it is never described as an accessibility guarantee.

`app/routing_resources.py` owns the ignored production-routing activation boundary. It accepts only the exact X1-reviewed Valhalla 3.8.3 executable, loopback configuration and 46,018,560-byte Gold Coast graph digests, the matching `au-qld-gold-coast` Spatial Pack version and reviewed coverage/source/cache declaration. `app/valhalla_adapter.py` rechecks resource bytes before launch, uses only `127.0.0.1`, sends coordinates, the explicit provisional speed and supported controls rather than canonical labels, independently limits endpoint snapping to 100 metres and validates returned route geometry against the declared extent. Provider time is treated as a static duration estimate; Project E applies Australia/Brisbane depart-at/arrive-by and named-buffer semantics because the graph lacks reviewed time-dependent/admin data.

`app/journey_cache.py` stores only bounded disposable results in a separate ignored SQLite file; fresh, stale and miss are explicit, invalid cache payloads are discarded and clearing it loses no personal configuration. The N6 caller fixes 128 entries and a 24-hour freshness window after representative result-size/lookup measurement. Walking results provide text and a temporary Map route overlay, but create no route history, reusable journey, Calendar Event or journey group; a stale candidate remains visible only when a current calculation fails.
...
`app/map_coverage_service.py` owns display-only coverage advice for any currently mapped point. It reads the active validated coverage GeoJSON, distinguishes reviewed core geometry from wider declared context bounds, measures point-to-boundary distance and retains the exact selected Map return URL. The review explains a same-region refresh, adjoining-area/common-snapshot union or separate-region candidate with current installed size/bounds as a non-linear baseline, declared source versions and explicit network consequences. It creates no database state, audit event, provider request, catalogue entry, build, download or installation; candidate acquisition and Spatial Pack preview remain separate user-controlled workflows.
...
Phases 1 and 2 are complete development milestones. Phase 3 N1–N6 has added the canonical place foundation, offline-first Map 2.0 workspace, provider-independent journey seam, one verified installed Gold Coast map/search/context pack, reviewed provider/list/coverage workflows and trustworthy local walking journeys. Static public transport, Calendar materialisation, broader routing modes and later spatial work remain separately gated. Mobile access, multi-user permissions, autonomous external effects and AI/agent layers require separate authorisation. Any future consumer—human interface, deterministic integration or bounded AI capability—must use the same canonical data, validation, audit, provenance and recovery boundaries.
