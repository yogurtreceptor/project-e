...
Phase 2 is in progress. Phase 2B work management is complete, and Phase 2C has begun with a local reminder-resolution service and durable Inbox. Calendar, Task-list and global derived-source reminder defaults are configurable locally, while record overrides add or suppress individual timings. Recurring Event reminder edits use the established occurrence, prospective-series and whole-series scopes; occurrence cancellation, truncation and prospective splits resolve only their affected pending delivery. Recycling a reminder source likewise resolves its active attention. The Inbox orders active attention with overdue Tasks first, derives a separate Upcoming reminder preview without creating delivery records, and presents the 500 most recent acted-on items through paged Archive plus a distinct deep-history view. It supports manual evaluation and local acknowledgement, dismissal and snooze actions, each recorded as an append-only delivery transition and shown on archived items; scheduled delivery and startup recovery remain Phase 2D. The later operational runtime adds registered scheduled jobs and deterministic automation. It does not add AI, agents, cloud service layers, external notification channels or autonomous external side effects. The detailed target architecture is in [the Phase 2 plan](phase_2_plan.md).
