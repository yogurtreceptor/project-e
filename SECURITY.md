...
Provider-feature review and Map-list forms are also untrusted browser input. The server allowlists supported provider identities, bounds text and coordinates, requires the named spatial-pack version to remain active and inside declared coverage, and re-resolves indexed pack features before review. Unindexed rendered context is labelled approximate. Promotion always requires a duplicate review and explicit canonical target; linking an existing Location can add only a provider reference. Portable list rows contain user-owned labels and provider identity, not submitted coordinates or provider snapshots.

Coverage-recommendation query values are untrusted display-only input. The server bounds and escapes the selected label and coordinates, sanitizes the return path to Map, and reads only the active local manifest/coverage geometry. The review cannot contact a source, acquire or activate a pack, mutate canonical or operational state, or treat declared rectangular bounds as reviewed administrative coverage. External source links are ordinary explicit navigation, never automatic resource requests.
