...
Project E is a local-first Personal Information Platform for one private user. It stores canonical People, Organisations, Locations, Projects, Documents, Assets and Events; first-class Relationships; and local operational records such as Calendars, reminders and Inbox deliveries. Phases 1 and 2 are complete. Phase 3 spatial-intelligence delivery is in progress with N1–N6 and the X1 Gold Coast prerequisite complete: the canonical place foundation, offline-first Map 2.0 workspace, provider-independent journey/profile/policy/cache foundation, verified replaceable Gold Coast local map/search/context pack, reviewed provider-to-Location promotion, portable external Map lists, contextual non-installing coverage recommendations and trustworthy local walking journeys are current architecture. N7 onward remains planned. Personal operational intelligence remains a longer-term direction.
...
- Phase 2's Calendar, Event, reminder, Inbox, scheduling and registered-automation behaviour remains current architecture. Phase 3 N1–N6's canonical address/geometry/containment, offline-first Map 2.0, provider-independent journey/profile/policy/cache foundation, one-region verified spatial-pack lifecycle, reviewed provider promotion, portable Map-list behaviour, contextual coverage review and local Valhalla walking adapter with a provisional 5 km/h Regular walk profile, disabled Jog/Run choices, supported policy and measured cache bounds are current architecture. X1's remaining transit front-runner is evidence only, and N7 onward remains evolving desired state rather than an implementation checklist.
...
- Experience and UI: `docs/experience_philosophy.md` for principles, then only the responsible file under `docs/design/`.
