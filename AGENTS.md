...
- Phase 2's Calendar, Event, reminder, Inbox, scheduling and registered-automation behaviour remains current architecture. Phase 3 N1–N6's canonical address/geometry/containment, offline-first Map 2.0, provider-independent journey/profile/policy/cache foundation, one-region verified spatial-pack lifecycle, reviewed provider promotion, portable Map-list behaviour, contextual coverage review and local Valhalla walking adapter with reviewed measured profiles/policy/cache bounds are current architecture. X1's remaining transit front-runner is evidence only, and N7 onward remains evolving desired state rather than an implementation checklist.
