...
def create_reference_data_tables(connection: object) -> None:
...
def seed_reference_data(connection: object) -> None:
    labels = {
        "country": "Countries",
    ...
    for type_key, name in labels.items():
        connection.execute(
            "INSERT INTO reference_data_types(key, name) VALUES (?, ?) ON CONFLICT DO NOTHING",
            (type_key, name),
        )
...
def list_reference_items(
    connection: object, type_key: str, *, include_inactive: bool = False
) -> list[ReferenceItem]:
...
def _to_reference_item(row: dict) -> ReferenceItem:
