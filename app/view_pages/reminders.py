...
def reminder_policy_page(title: str, back_url: str, *, configured_timings: list[str] | None, inherited_timings: list[str]) -> str:
    ...
    timings = ", ".join(configured_timings if configured else inherited_timings)
    return f'''<section class="page-heading"><p class="eyebrow">Operational attention</p><h1>{escape(title)}</h1><p>Set the reminder timings used by this context. Restoring inheritance uses the next applicable default.</p></section><section class="panel"><form class="record-form" method="post"><label><span>Policy</span><select name="mode"><option value="custom"{" selected" if configured else ""}>Use these timings</option><option value="inherit"{" selected" if not configured else ""}>Use inherited defaults</option></select></label><label><span>Timings</span><input name="timings" value="{escape(timings)}" placeholder="e.g. 2h, 1d"{" required" if configured else ""}></label><p class="help-text">Use comma-separated positive timing tokens: m, h, d, w or mo.</p><p class="help-text">Inherited timings: {escape(", ".join(inherited_timings))}.</p><div class="actions"><a class="button secondary" href="{escape(back_url)}">Cancel</a><button class="button">Save reminder policy</button></div></form></section>'''
