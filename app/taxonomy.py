...
def create_taxonomy_tables(connection: object) -> None:
    connection.executescript(
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_taxonomy_sibling_label
            ON taxonomy_entries(taxonomy_id, COALESCE(parent_id, 0), lower(label));
...
def _ensure_column(connection: object, table: str, name: str, declaration: str) -> None:
    columns = {row["name"] for row in connection.execute(f"PRAGMA table_info({table})")}
    if name not in columns:
        connection.execute(f"ALTER TABLE {table} ADD COLUMN {name} {declaration}")


def _taxonomy_id(connection: object, key: str) -> int:
...
def seed_taxonomies(connection: object) -> None:
    organisation = (
    ...
    for item in RELATIONSHIP_TYPES:
        ...
        duplicate = connection.execute("SELECT 1 FROM taxonomy_entries WHERE taxonomy_id=? AND parent_id=? AND lower(label)=lower(?)", (_taxonomy_id(connection, "relationship_type"), parent_row["id"], leaf)).fetchone()
        if duplicate:
...
def migrate_taxonomy_assignments(connection: object) -> None:
...
def list_entries(connection, taxonomy_key: str, include_archived: bool = False) -> list[TaxonomyEntry]:
    rows = connection.execute(
        ...
        (taxonomy_key, taxonomy_key, include_archived),
    ).fetchall()
...
def create_entry(connection, taxonomy_key: str, label: str, parent_id: int | None = None, relationship=None) -> int:
    ...
    duplicate = connection.execute(
        """SELECT 1 FROM taxonomy_entries
           WHERE taxonomy_id=? AND parent_id IS NOT DISTINCT FROM ?
             AND lower(label)=lower(?)""",
        (taxonomy_id, parent_id, label),
    ).fetchone()
    if duplicate:
        raise ValueError("A sibling taxonomy entry already uses that label.")
