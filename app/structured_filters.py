...
FilterPredicate = Callable[[Any, EntityRecord, str], bool]
...
def _birthday_month(_connection: object, record: EntityRecord, value: str) -> bool:
    birthday = record.metadata.get("birthday", "")
    return bool(value and len(birthday) >= 7 and birthday[5:7] == value.zfill(2))


def _birth_year(_connection: object, record: EntityRecord, value: str) -> bool:
    return bool(value and record.metadata.get("birthday", "").startswith(value + "-"))


def _no_birthday(_connection: object, record: EntityRecord, _value: str) -> bool:
    return not record.metadata.get("birthday", "")


def _organisation_no_location(connection: object, record: EntityRecord, _value: str) -> bool:
...
def apply_structured_filter(connection: object, records: list[EntityRecord], key: str, value: str = "") -> list[EntityRecord]:
