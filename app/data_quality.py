...
def date_sanity(c):
 ...
 for r in list_all_entities(c):
  for name,value in r.metadata.items():
   if (name.endswith("date") or name in {"birthday","started_at","ended_at"}) and name not in {"expiry_date","target_date"} and value>utc_now()[:10]:out.append(finding("date_sanity","warning",f"{r.title} has a future {name}.",(r.id,)))
 return out
@registry.register
