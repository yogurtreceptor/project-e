...
registry.register("relationship",lambda c,r,v:bool(c.execute("SELECT 1 FROM relationships x JOIN entities e ON e.id=CASE WHEN x.source_entity_id=? THEN x.target_entity_id ELSE x.source_entity_id END WHERE x.deleted_at='' AND x.type=? AND (x.source_entity_id=? OR x.target_entity_id=?) AND e.deleted_at=''",(r.id,v,r.id,r.id)).fetchone()))
def related(kind):return lambda c,r,v:bool(c.execute("SELECT 1 FROM relationships x JOIN entities e ON e.id=CASE WHEN x.source_entity_id=? THEN x.target_entity_id ELSE x.source_entity_id END WHERE x.deleted_at='' AND (x.source_entity_id=? OR x.target_entity_id=?) AND e.type=? AND e.deleted_at='' AND lower(e.display_name) LIKE ?",(r.id,r.id,r.id,kind,"%"+v.lower()+"%")).fetchone())
