...
class DesignFoundationTests(unittest.TestCase):
    ...
    def test_quick_create_script_is_allowed_by_static_handler(self) -> None:
        ...
        try:
            with urlopen(
                f"http://127.0.0.1:{server.server_port}/static/quick-create.js",
                timeout=5,
            ) as response:
                self.assertEqual(response.status, 200)
    ...
    def test_mini_month_picker_script_is_allowed_by_static_handler(self) -> None:
        ...
        try:
            with urlopen(
                f"http://127.0.0.1:{server.server_port}/static/mini-month-picker.js",
                timeout=5,
            ) as response:
                self.assertEqual(response.status, 200)
                script = response.read()
                self.assertIn(b"ArrowLeft", script)
                self.assertIn(b"PageUp", script)
