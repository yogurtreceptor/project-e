...
Consequences:
The first production slice deliberately supports one active region and a small indexed `LIKE` search rather than a general pack federation, FTS platform or native geocoder service. Coordinates and canonical pins remain visible outside pack coverage or when the renderer/search fails. Map source inputs, derived indexes and versions must be reacquired/reinstalled independently of personal recovery. Reconsider multi-region composition when N6 route-source compatibility supplies a concrete union case; reconsider FTS or MOTIS search only when representative search semantics/latency exceed the fixed index; reconsider PMTiles only on a measured MBTiles serving or lifecycle problem. N5 subsequently delivered reviewed duplicate-aware provider promotion, portable lists and provider-version disappearance/reconciliation under ADR-034 plus non-installing contextual coverage advice under ADR-035.

## ADR-034: Separate reviewed provider acceptance from portable external membership
...
Decision:
Keep provider browsing non-mutating and route canonical promotion through one dedicated review. Revalidate current spatial-pack identity, show exact provider-reference, normalized name/address and representative-point proximity evidence, and require the user to choose a new Location or a reviewed existing match. Creating new records source-qualified address/point assertions plus a versioned neutral provider reference; choosing existing adds only the reference and never overwrites canonical identity or assertions.

Store external favourites and named lists in durable `map_feature_lists` / `map_feature_list_memberships` rows. A membership owns only list identity, provider key, feature identity and a user-owned label. Resolve current provider facts for display without persisting version, coordinates or source snapshots; retain and label membership as unavailable when its pack/provider/feature disappears. Require an explicit online search to revisit an online membership. Migration `20260808_35_map_feature_lists` carries this user state through whole-platform recovery while pack bytes remain excluded.

Reason:
Two fictional pack versions proved the important split. A changed feature identity could still be reconciled to the same canonical Location through visible name/proximity evidence and a second accepted provider reference, without moving its first geometry. The old list identity became unavailable after update while its user membership and label survived both provider disappearance and export/import. Automatically copying provider facts into list rows would make the portable database a stale provider cache; automatically matching the new version would conceal ambiguous replacement.

Consequences:
Provider removal cannot delete list membership or canonical Locations. List revisit is useful when the exact current identity resolves and honestly degraded otherwise. Rendered features not present in the fixed search index retain an approximate clicked point during review and cannot masquerade as indexed source geometry. Automatic fuzzy reconciliation, provider-fact snapshots, list import/merge, recent-search retention and bordering-region coverage recommendations are not introduced by this decision; ADR-035 subsequently adds only the display-only coverage recommendation. Reconsider the exact identity or proximity strategy only with measured false matches or a provider offering a stronger stable identifier; reconsider persisted observation facts only if a concrete offline workflow cannot be served by reacquisition without confusing them with canonical truth.

## ADR-035: Derive coverage advice from reviewed core geometry without acquiring resources
...
Decision:
Expose **Improve coverage** for every mapped point and retain its exact Map return URL. Read the active validated `coverage.geojson` and manifest locally, classify the point as inside reviewed core geometry, inside declared context bounds only or outside pack bounds, and show measured distance where the distinction matters. Recommend a same-region refresh, adjoining-area/common-snapshot union or separate bounded region accordingly.

Explain current installed member size and rectangular-bounds area as a baseline, never as a linear candidate estimate. State that candidate archive/unpacked size must be measured during Spatial Pack preview, network acquisition is separate and explicit, and new administrative coverage needs an appropriate reviewed boundary source. The review itself performs no provider request, catalogue lookup, build, download, activation, persistence, audit write or canonical mutation.

Reason:
The verified Gold Coast pack has a 23.6 MB installed member footprint over approximately 6,343 km² of declared rectangular bounds, but that rectangle contains a 15 km context buffer rather than administrative truth. The X1 northern boundary point is 1.1 km beyond the official Gold Coast core while still inside context. The Brisbane failure point is 9.5 km beyond pack bounds and 32.3 km from the reviewed core. These cases prove that a useful recommendation can distinguish bordering context from genuinely separate coverage without inventing a neighbouring-LGA name, extrapolating size from area or selecting a provider.

Consequences:
Missing detail inside core suggests a same-scope refresh, not automatic expansion. Context-only points remain visibly outside reviewed administrative coverage even when tiles exist. Outside points retain selection while describing a separate pack or evidence-backed union; Project E still supports only one active region and offers no pack catalogue or installer from this page. Future route coverage must assess the route corridor through the same boundary distinction when calculated route geometry exists; endpoints alone cannot certify corridor coverage. Reconsider persisted recommendation state only if a concrete user workflow cannot retain context through the existing Map URL.

## ADR-036: Adopt one exact local Valhalla walking capability with a provisional Regular profile
...
Decision:
Adopt Valhalla 3.8.3 for N6 walking only, behind the N3 adapter contract and an ignored `project-e-local-routing` activation declaration. Activation accepts the exact X1-reviewed service/configuration/Gold Coast graph digests, requires the matching `au-qld-gold-coast` Spatial Pack version and binds only to `127.0.0.1`. The adapter rechecks bytes before launch, sends deliberate endpoint coordinates plus the supported explicit configuration, refuses endpoint snaps beyond 100 metres and labels any returned corridor outside the declared rectangular extent as partial.

Initialise one audited Regular walk profile on first use with a provisional generic speed of 5 km/h (`1.388889 m/s`). Record that the source is a Google Maps Community Product Expert estimate: Google does not publish a fixed Maps walking-speed constant in its developer documentation, and the answer itself notes route-condition effects. Display the value as generic rather than personal. Keep Fast walk/jog and Run visibly disabled and reject them in both the request parser and adapter. Support only an explicitly enabled strong soft preference to avoid steps, mapped to Valhalla's high step transition penalty. Do not map unproved accessibility, safety, lighting, surface, slope or live-condition claims. Treat route duration as a static estimate and apply Project E's Australia/Brisbane depart-at/arrive-by plus preparation/arrival-buffer semantics outside the provider.

Fix the disposable walking-result cache at 128 entries and 24-hour freshness for this immutable street snapshot. Continue to fingerprint every request, canonical geometry, profile/policy revision and definition, adapter, coverage and source version. Text and temporary Map geometry are result projections only; no calculation or stale candidate creates an Event, route history, reusable journey or journey group.

Reason:
The X1 graph is 46,018,560 bytes and the local process fitted the existing standard-library loopback boundary. A live representative request produced a 3,774-byte normalized result; the second lookup took 3.9 ms and made zero adapter calls, so 128 result payloads are roughly 0.46 MiB before SQLite overhead. Source/revision fingerprint tests proved deterministic invalidation. The independently bounded locate check rejected the earlier distant-snap behavior. On the reviewed Miami stair example, normal walking returned 184 m/148 s while the strong soft preference returned a 1,402 m/1,133 s route reported step-free by the provider maneuvers. That is useful policy evidence, but it does not establish accessibility or safety.

Consequences:
Walking works without WAN when the exact ignored resources and matching pack are present, and otherwise fails visibly while Map and canonical Locations remain available. Resource changes require a new reviewed declaration and adapter/source version rather than an in-place trust expansion. WSL-local execution is the measured deployment; reconsider MOTIS or native Valhalla packaging only if this runtime boundary becomes operationally inadequate. Reconsider the provisional 5 km/h value, disabled faster profiles, 100-metre snap limit, cache bounds/freshness or step penalty only with representative evidence or explicit user direction. N7 transit, N8 Calendar materialisation, N9 drive/cycle breadth, accessibility profiles and adjoining-region routing remain separate authorisation gates.
