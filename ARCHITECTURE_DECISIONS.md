...
Consequences:
The first production slice deliberately supports one active region and a small indexed `LIKE` search rather than a general pack federation, FTS platform or native geocoder service. Coordinates and canonical pins remain visible outside pack coverage or when the renderer/search fails. Map source inputs, derived indexes and versions must be reacquired/reinstalled independently of personal recovery. Reconsider multi-region composition when N5 coverage recommendations or N6 route-source compatibility supplies concrete adjoining-region cases; reconsider FTS or MOTIS search only when representative search semantics/latency exceed the fixed index; reconsider PMTiles only on a measured MBTiles serving or lifecycle problem. N5A subsequently delivered reviewed duplicate-aware provider promotion, portable lists and provider-version disappearance/reconciliation under ADR-034; contextual coverage recommendations remain N5 work.
...
Consequences:
Provider removal cannot delete list membership or canonical Locations. List revisit is useful when the exact current identity resolves and honestly degraded otherwise. Rendered features not present in the fixed search index retain an approximate clicked point during review and cannot masquerade as indexed source geometry. Automatic fuzzy reconciliation, provider-fact snapshots, list import/merge, recent-search retention and bordering-region coverage recommendations are not introduced by this decision. Reconsider the exact identity or proximity strategy only with measured false matches or a provider offering a stronger stable identifier; reconsider persisted observation facts only if a concrete offline workflow cannot be served by reacquisition without confusing them with canonical truth.
