...
## ADR-036: Adopt one exact local Valhalla walking capability with measured user profiles
...
Decision:
Adopt Valhalla 3.8.3 for N6 walking only, behind the N3 adapter contract and an ignored `project-e-local-routing` activation declaration. Activation accepts the exact X1-reviewed service/configuration/Gold Coast graph digests, requires the matching `au-qld-gold-coast` Spatial Pack version and binds only to `127.0.0.1`. The adapter rechecks bytes before launch, sends deliberate endpoint coordinates plus supported measured configuration, refuses endpoint snaps beyond 100 metres and labels any returned corridor outside the declared rectangular extent as partial.

Keep Regular walk, Fast walk/jog and Run as stable Walk-profile identities with no built-in personal speed. Each becomes usable only after review of three trials over one known distance; the median speed and optional contiguous distance/duration limits are revisioned user-owned configuration. Support only an explicitly enabled strong soft preference to avoid steps, mapped to Valhalla's high step transition penalty. Do not map unproved accessibility, safety, lighting, surface, slope or live-condition claims. Treat route duration as a static estimate and apply Project E's Australia/Brisbane depart-at/arrive-by plus preparation/arrival-buffer semantics outside the provider.
